...
public class EntitiesPacketHandler extends PacketHandler {
    ...
    private void refreshTextDisplayEntities(@NotNull SpigotLanguagePlayer languagePlayer, @NotNull Player bukkitPlayer) {
        ...
        for (Map.Entry<Integer, String> entry : entitiesInCurrentWorld.entrySet()) {
            ...
            val textComponent = ComponentUtils.deserializeFromJson(text);
            val result = parser()
                    .translateComponent(
                            textComponent,
                            languagePlayer,
    ...
    private class DataValueHandler {
        ...
        Optional<WrappedDataValue> getTextDisplayTextDataValue(@Nullable Component component) {
            Object payload;
            if (component != null) {
                payload = WrappedComponentUtils.serialize(component).getHandle();
            } else {
        ...
        Optional<WrappedDataValue> translateTextDisplayTextDataValue(
            ...
            val textJson = ((WrappedChatComponent) value).getJson();

            // Save to cache before translating
            saveToCache.accept(textJson);

            val textComponent = WrappedComponentUtils.deserialize((WrappedChatComponent) value);

            val result = parser()
                    .translateComponent(textComponent, languagePlayer, getConfig().getHologramSyntax());
            ...
            return this.getTextDisplayTextDataValue(result.getResult().orElse(null));
        }
    }
}
